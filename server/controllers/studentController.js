...
exports.updateStudentProfile = async (req, res) => {
    ...
    // DEBUG LOG 1: Check if we are getting here
    const targetUserId = student.user.toString();
    console.log(`[SERVER DEBUG] Status changed to ${status}. Notification target Room ID: ${targetUserId}`);

    // Emit
    io.to(targetUserId).emit("receive_notification", {
        id: Date.now(),
        title: "Profile Updated",
        message: `Your profile status has been changed to ${status.toUpperCase()}.`,
        type: status === 'verified' ? 'success' : 'info',
    ...
    // DEBUG LOG 2: Confirm emit executed
    console.log("[SERVER DEBUG] Notification emitted.");
}
        // ------------------------------------
