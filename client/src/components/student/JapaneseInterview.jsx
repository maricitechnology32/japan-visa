...
// import { AlertCircle, Briefcase, CheckCircle2, Loader2, Mic, Square, User } from 'lucide-react';
...
//     vapi.on('call-start', () => {
//       setStatus('active');
//       toast.success("Connected! The officer will speak first.");
//     });
...
//       if (msg.includes("ejected") || msg.includes("ended")) {
//           toast.error("Call rejected. Please try again.");
//       } else {
...
//       // 2. Define Context based on selection
//       let systemPrompt = "";
//       let firstMessage = "";
...
//         firstMessage = "こんにちは。入国審査を始めます。まず、お名前を教えてください。"; // Hello. Starting immigration check. Name please.
//       } else {
...
//         firstMessage = "初めまして。面接を担当します。自己紹介をお願いします。"; // Nice to meet you. Introduce yourself.
//       }

//       // 3. Start Call with INLINE CONFIGURATION
//       // This bypasses the Assistant ID issues and gives you full control
//       await vapiRef.current.start({
//         model: {
//             provider: "openai",
//             model: "gpt-4o", // Standard, reliable model
//             messages: [
//                 {
//                     role: "system",
...
//         transcriber: {
//             provider: "deepgram",
//             model: "nova-2", // Standard fast model
//             language: "ja" // Japanese
//         },
//         voice: {
//             provider: "11labs",
//             voiceId: "cgSgspJ2msm6clMCkdW9", // "Jessica" - Reliable voice
//             stability: 0.5,
...
//       {/* 1. HEADER & VISUALIZER */}
//       <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center relative overflow-hidden">
//         {status === 'active' && (
//              <div className="absolute top-4 right-4 bg-red-100 text-red-600 px-3 py-1 rounded-full text-xs font-bold animate-pulse flex items-center gap-2">
...
//                 status === 'active' 
//                     ? isSpeaking ? 'border-green-400 bg-green-50 scale-110' : 'border-red-500 bg-red-50'
...
//                              <div 
//                                 key={i} 
//                                 className={`w-2 rounded-full transition-all duration-75 ${isSpeaking ? 'bg-green-500' : 'bg-red-500'}`}
//                                 style={{
//                                     height: isSpeaking ? `${Math.max(20, Math.random() * volume * 400)}%` : '20%',
//                                     animation: isSpeaking ? 'none' : 'bounce 1s infinite'
//                                 }} 
//                              />
//                         ))}
//                     </div>
...
//         <h2 className="text-2xl font-bold text-gray-900">AI Japanese Interviewer</h2>
//         <p className="text-gray-500 mt-2 max-w-md mx-auto">
//             {status === 'active' 
//                 ? "The officer is listening. Please answer in Japanese." 
//                 : "Select a scenario below to begin your mock interview."}
//         </p>
//       </div>

//       {/* 2. SCENARIO SELECTION (Disable during call) */}
//       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//           {/* Student Card */}
//           <div 
//             onClick={() => status === 'idle' && setActiveScenario('student')}
...
//                   <div className={`p-3 rounded-lg ${activeScenario === 'student' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
//                       <User size={24} />
//                   </div>
//                   {activeScenario === 'student' && <CheckCircle2 className="text-green-600" size={24} />}
//               </div>
//               <h3 className="font-bold text-lg text-gray-900">Student Visa</h3>
//               <p className="text-sm text-gray-500 mt-1">Immigration officer persona. Questions about school, sponsor, and study plans.</p>
//           </div>

//           {/* Work Card */}
//           <div 
//             onClick={() => status === 'idle' && setActiveScenario('work')}
//             className={`p-6 rounded-xl border-2 cursor-pointer transition-all ${
//                 activeScenario === 'work' 
...
//           >
//               <div className="flex items-start justify-between mb-3">
//                   <div className={`p-3 rounded-lg ${activeScenario === 'work' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
//                       <Briefcase size={24} />
//                   </div>
//                   {activeScenario === 'work' && <CheckCircle2 className="text-purple-600" size={24} />}
//               </div>
//               <h3 className="font-bold text-lg text-gray-900">Work Visa</h3>
//               <p className="text-sm text-gray-500 mt-1">Factory Manager persona. Questions about skills, experience, and motivation.</p>
//           </div>
//       </div>
...
//                 className={`px-8 py-4 rounded-full font-bold text-lg text-white shadow-lg flex items-center gap-3 transition-all transform hover:scale-105 active:scale-95 ${
//                     activeScenario === 'student' ? 'bg-green-600 hover:bg-green-700 shadow-green-200' : 'bg-purple-600 hover:bg-purple-700 shadow-purple-200'
//                 }`}
//              >
//                 <Mic size={20} /> 
//                 Start {activeScenario === 'student' ? 'Student' : 'Work'} Interview
//              </button>
//          ) : (
//              <button 
//                 onClick={stopInterview}
//                 className="bg-white border-2 border-red-100 text-red-600 hover:bg-red-50 px-10 py-4 rounded-full font-bold text-lg shadow-sm flex items-center gap-3 transition"
//              >
//                 <Square size={20} fill="currentColor" /> End Interview
//              </button>
//          )}
//       </div>

//       <div className="mt-8 flex items-center justify-center gap-2 text-xs text-gray-400 bg-gray-50 p-3 rounded-lg max-w-md mx-auto">
//          <AlertCircle size={14} />
//          <span>Ensure your microphone is enabled. The AI will speak first.</span>
//       </div>
